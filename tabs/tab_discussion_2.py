...
GUESTS_FILE = "data/guest_users.json"
...
def save_guests(guests_list):
    os.makedirs(os.path.dirname(GUESTS_FILE), exist_ok=True)
    with open(GUESTS_FILE, 'w', encoding='utf-8') as f:
        json.dump(guests_list, f, ensure_ascii=False, indent=4)

guest_users = load_guests()
...
def render_discussion():
    ...
    if st.session_state.current_user is None:
        st.info("👋 Hãy chọn username nếu đã mua vé hoặc tạo username mới để tham gia bình luận!")
        
        # Create two tabs: Dropdown for existing/ticket users, Text input for new guests
        tab_login, tab_register = st.tabs(["🔐 Đăng nhập (với user đã mua ticket)", "📝 Tạo username mới"])
        
        # --- TAB 1: LOGIN (CHỌN TÊN CÓ SẴN) ---
        with tab_login:
            with st.form(key="login_form"):
                # Load ticket holders from CSV
                df = pd.read_csv(r"data/standing.csv").dropna(how='all')
                ticket_holders = df['ticket'].str.split(" - ").str[0].drop_duplicates().tolist()
                
                # Combine ticket holders + previously registered guests into one dropdown
                all_usernames = sorted(list(set(ticket_holders + guest_users)), key=lambda x: unidecode(x))
                
                username_input = st.selectbox('Lựa chọn tên:', all_usernames)
        ...
        # --- TAB 2: REGISTER (TẠO TÊN MỚI) ---
        with tab_register:
            with st.form(key="register_form"):
                st.caption("Dành cho username chưa có ticket.")
                
                # The text input with your requested placeholder
                reg_display = st.text_input("Tên hiển thị:", placeholder="VD: Đoàn Thị Hiền")
                
                submit_register = st.form_submit_button("Đăng ký & Đăng nhập")
                
                if submit_register:
                    clean_name = reg_display.strip()
                    if not clean_name:
                        st.warning("⚠️ Vui lòng nhập tên hiển thị!")
                    else:
                        # Re-load ticket holders to check if the name already exists
                        df = pd.read_csv(r"data/standing.csv").dropna(how='all')
                        ticket_holders = df['ticket'].str.split(" - ").str[0].drop_duplicates().tolist()
                        
                        # If the name is completely new, save it to our guests JSON file
                        if clean_name not in ticket_holders and clean_name not in guest_users:
                            guest_users.append(clean_name)
                            save_guests(guest_users)
                        
                        # Log them in automatically
                        st.session_state.current_user = clean_name
                        st.rerun()
    else:
        # If already logged in, show the welcome message and logout button
        col1, col2 = st.columns([3, 1])
        with col1:
            st.success(f"👤 Đang đăng nhập dưới tên: **{st.session_state.current_user}**")
        with col2:
